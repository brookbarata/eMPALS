...
class MissingController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if($validator->fails()){
           return redirect('user/report-missing', )->with('danger', 'Error! Please Enter Some Valid Inputs.');
     }
     else{
          ...
          \Session::put('_photo','/storage/'.$path);   
            ...
            if($data['results']->isNotEmpty()){
                return view('user/suggestions/suggestions-missing', $data)->with('success', 'Reports Frist Part added, go ahead.');
            }
            else{
