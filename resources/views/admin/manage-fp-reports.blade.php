...
@section('content')
  ...
  <div class="content-wrapper">
    ...
    <div class="content-header">
      <div class="container">
        <div class="row">
          <div class="col-sm-6">
            <h1 class="m-0">Manage <span class="fw-bold text-success">Found</span> Persons</h1>
          </div><!-- /.col -->
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
            <li class="breadcrumb-item"><a href="/admin/dashboard">Dashboard</a></li>
              <li class="breadcrumb-item active">Manage Found Persons</li>
            </ol>
          </div><!-- /.col -->
        </div><!-- /.row -->
      </div><!-- /.container-fluid -->
  ...
  <table class="table">
    ...
    @foreach ($found_report as $item )      
      <tr>
        ...
        @else 
        <td>
        <form method="POST" action="{{ route('manage-found-person.update', $item)}}" onsubmit= "return confirm('Validate?')"> 
            @csrf  
            @method('PUT')  
        ...
        @if( $item->notified == 0)
        <td>
        <form method="POST" action="{{ route('send-mail.update', $item)}}" onsubmit= "return confirm('Notfiy Nearby Police/Volunteers?')"> 
            @csrf  
            @method('PUT')  
        ...
        <td>
          <ul class="list-inline sm-12 m-0">
                  ...
                  <form method="POST" action="{{ route('manage-found-person.destroy', $item)}}"  onsubmit= "return confirm('Delete?')"> 
                      @csrf  
                      @method('DELETE')
  ...
  @else
  <div>
  <p class=" px-5 text-danger">Oh, Noops! You don't have any PENDING found person reports to manage.</p>
  </div>
  @endif
